...
export default function ViewProposalsPage() {
  ...
  const handleProposalClick = (id) => {
    router.push(`/user/proposals/${id}`);
  };

  const handleEditClick = (e, id) => {
    e.stopPropagation(); // Prevent card click from firing
    router.push(`/user/proposals/${id}/edit`);
  };
        ...
        <CardContent className="flex flex-col flex-grow justify-between">
          <div>
            <p className="flex items-center text-sm text-slate-400">
              <User className="h-4 w-4 mr-2 flex-shrink-0 text-slate-500" />
              {proposal.currentReviewer.level === 2 ? (
                <span className="font-medium text-blue-300 ml-1.5 truncate bg-blue-500/10 rounded-md px-2 py-1 w-fit">
                  {"TCW Reviewer"}
                </span>
              ) : (
                <span className="font-medium text-blue-300 ml-1.5 truncate bg-blue-500/10 rounded-md px-2 py-1 w-fit">
                  {proposal.currentReviewer?.name || "N/A"}
                </span>
              )}
            </p>
          </div>
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 text-white h-full flex flex-col">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
        ...
        <Button
          onClick={() => router.push("/user/proposals/add")}
          className="bg-blue-600 hover:bg-blue-700 transition-all shadow-md hover:shadow-lg rounded-full px-6 py-3"
        >
